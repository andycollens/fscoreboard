...
### Спец-треки для награждения (service)
```bash
# Файлы должны отдаваться как статика (после установки пути могут отличаться)
curl -I "http://localhost:3001/public/sound/special/fanfare.mp3"
curl -I "http://localhost:3001/public/sound/special/anthem-russia.mp3"
curl -I "http://localhost:3001/public/sound/special/champions.mp3"
# Ожидаемый результат: HTTP/1.1 200 OK (или 206 при Range; не 404)
```
...
- [ ] После применения пресета на `service.html` у **обеих** команд совпадают названия, составы и **логотипы** (не картинка «от прошлого матча»)
- [ ] Редактирование существующей предустановки
- [ ] Удаление предустановки

### Страница service (составы и награждение)
- [ ] Открывается с токеном: `service.html?token=SERVICE_TOKEN`
- [ ] Кнопка **Награждение** открывает всплывающее окно; громкость влияет на воспроизведение
- [ ] **Фанфары** / **Гимн РФ** / **Champions** не стартуют поверх уже играющего соседнего трека в этом окне (нужна остановка текущего)
- [ ] Закрытие оверлея по фону/Escape не срывает активное воспроизведение (ожидаемое поведение: пока играет — не закрывается)
